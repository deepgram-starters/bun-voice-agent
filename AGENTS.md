...
Bun demo app for Deepgram Voice Agent.

## Architecture

- **Backend:** Bun (TypeScript) on port 8081
...
# Terminal 2 — Frontend
cd frontend && bun run dev -- --port 8080 --no-open
```
...
- **Backend:** `package.json` — Uses Bun runtime and package manager. TypeScript runs natively.
...
```
feat(bun-voice-agent): add diarization support
fix(bun-voice-agent): resolve WebSocket close handling
refactor(bun-voice-agent): simplify session endpoint
